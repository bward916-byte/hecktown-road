...
const WORLD_DEFS = {
  ...
  taunton: {
    ...
    solids: () => [[0, 440, 3800, 200, 0], [1520, 392, 560, 48, 0], [1480, 416, 40, 24, 0], [2480, 434, 360, 6, 1], [3340, 288, 420, 92, 0], [3760, 328, 64, 54, 0], [3360, 412, 80, 28, 1], [3440, 410, 80, 30, 1], [3520, 382, 80, 58, 1], [728, 330, 60, 6, 1], [1140, 316, 100, 8, 1]],
...
function renderDrive() {
  ...
  // spinning wheel hubs
  ctx.strokeStyle = '#9aa0a8'; ctx.lineWidth = 2; for (const wx of [120 + 72, 120 + 118, 120 + 420 + 24, 120 + 420 + 74]) { for (let k = 0; k < 4; k++) { const a = drive.wheel + k * Math.PI / 2; ctx.beginPath(); ctx.moveTo(wx, ry - 20); ctx.lineTo(wx + Math.cos(a) * 14, ry - 20 + Math.sin(a) * 14); ctx.stroke(); } }
  ...
  { const wx = 120 + 420 + 36, wy2 = 270 + 6, ww = 40, wh = 28; ctx.save(); ctx.beginPath(); rr(ctx, wx, wy2, ww, wh, 3); ctx.clip(); ctx.fillStyle = '#1c2a3a'; ctx.fillRect(wx, wy2, ww, wh); ctx.save(); ctx.translate(wx + 16, wy2 + 46); ctx.scale(0.6, 0.6); drawHero(ctx, hero, 0, 0, 1, { t: gameTime, run: 0, moving: false }, null); ctx.restore(); ctx.save(); ctx.translate(wx + 4, wy2 + 26); ctx.scale(0.55, 0.55); drawBiscuit(ctx, { x: 0, y: 0, facing: 1, moving: false, run: 0, t: gameTime, earA: -0.4 + Math.sin(gameTime * 8) * 0.2 }, null); ctx.restore(); ctx.restore(); }
  // headlights at night
