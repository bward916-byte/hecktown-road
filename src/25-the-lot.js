...
const _loadWorld17 = loadWorld; loadWorld = function (id, at) { _loadWorld17(id, at); lotCar.on = false; if (WORLD.def && WORLD.def.lotX && !demo.active) { const lx = WORLD.def.lotX; if (!npcs.some(n => n.look && n.look.name === 'Gate')) addNPC({ look: { name: 'Frank', role: 'Nights · gate', skin: '#e8b48e', hair: '#8e8e8e', style: 'cap', shirt: '#4a5060', pants: '#2b2f3a', acc: 'keys' }, x: lx + 60, y: 440, facing: 1, lines: ['Frank: "Employee lot. Badge in, badge out. The RV at the end is not mine and I have questions."', 'Frank: "Nobody parks in the far row. Nobody knows why. I know why."'] }); } };
